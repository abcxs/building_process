...
def batShp2txt(thread_id, files, output_dir, convert_results):
    for key, file in files.items():
        ...
        if backgroud:
            for num_id, img_file in enumerate(img_files):
                output_img_path = os.path.join(output_dir, 'tmp', '%d_%d.jpg' % (id_, num_id))
                output_txt_path = os.path.join(output_dir, 'tmp', '%d_%d.txt' % (id_, num_id))
                shutil.copyfile(img_file, output_img_path)
                with open(output_txt_path, 'w') as f:
                    f.write('')
                convert_files.append({'img_path': output_img_path, 'txt_path': output_txt_path})
        else:
            num_id = 0
            for img_file in img_files:
                for ann_file in ann_files:
                    print(thread_id, id_, img_file, ann_file)
...
def multiShp2txt(cfg):
    ...
    for i in range(num_process):
        ...
        p = multiprocessing.Process(target=batShp2txt, args=(i, process_files, output_dir, convert_results))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
