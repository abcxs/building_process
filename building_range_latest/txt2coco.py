...
def divide(process_id, files, cfg):
    ...
    for k, v in files.items():
        ...
        id_ = v['id']
        print('process:', process_id, 'id:', id_, 'path:', k)
        ...
        for convert_result in convert_results:
            ...
            for row in range(0, height, size - overlap_size):
                for col in range(0, width, size - overlap_size):
                    ...
                    assert start_row >= 0 and start_col >= 0, 'start_row: {}, start_col: {}'.format(start_row, start_col)
                    ...
                    if padding:
                        ...
                        tile_img = img[start_row: start_row + tile_height, start_col: start_col + tile_width]
                    ...
                    data_item = {}
                    image = {}
                    file_name = '%s_%s_%d_%d' % (prefix, img_name, start_row, start_col)
                    ...
                    cv2.imwrite(os.path.join(img_output_dir, '%s.png' % file_name), temp)

                    temp = temp.copy()
                    for box in boxes:
                        box = np.array(box, dtype=np.int32)
                        cv2.rectangle(temp, (box[0], box[1]), (box[2] + box[0] - 1, box[3] + box[1] - 1), (255, 0, 0), 1)
                    for segmentation in segmentations:
                        segmentation = np.array(segmentation, dtype=np.int32)
                        cv2.polylines(temp, [segmentation.reshape(-1, 1, 2)], True, (0, 0, 255), 1)
                    cv2.imwrite(os.path.join(visual_output_dir, '%s.png' % file_name), temp)
...
def multi_divide(cfg):
    ...
    with open(os.path.join(output_dir, 'filelist.json')) as f:
        files = json.load(f)
    ...
    for i in range(num_process):
        ...
        p = multiprocessing.Process(target=divide, args=(i, process_files, cfg))
        p.start()
        process.append(p)
    for p in process:
        p.join()
    ...
    for split in ['train', 'val']:
        ...
        result['categories'].append({'supercategory': 'none', 'id': 1, 'name': 'building'})
        ...
        with open(os.path.join(output_dir, split, '%s.json' % split), 'w') as f:
            json.dump(result, f)
